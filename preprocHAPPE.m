function preprocHAPPE(datafile, samplingRate, channels2use, channelLocFile)
%   preprocHAPPE - will run HAPPE preprocessing pipeline on HBN raw data
%
%   INPUT
%       datafile = full filepath and name to raw data
%       samplingRate = sampling rate (e.g., 500)
%       channels2use = cell array of names of channels to select
%       channelLocFile = full filepath and name to file with channel
%       locations information
%

%% initial parsing of arguments to variables that HAPPE code will later use
[fpath, fname, fext] = fileparts(datafile);
src_folder_name = fpath;

% channel IDs
chan_IDs = channels2use;

% channel location file
chan_locations = channelLocFile;


%% DEFAULTS

% Which acquisition layout would you like to use?
% EGI Geodesic Sensor Net 64 Channel v2.0: layout_type = 1;
% EGI Hydrocel Geodesic Sensor Net 128 v1.0 : layout_type = 2;
layout_type = 2;


% run HAPPE in semi-automated setting with visualizations (=1) or fully-automated, no visualizations setting ( = 0)
...
% for resting-state EEG, set task_EEG_processing = 0
...
potential_eeg_var_names = {'EEG','data'};


% do you want to segment your data? yes (=1) or no (=0)
segment_data = 0;
...
% do you want to interpolate the specific channels' data determined to be artifact/bad within each segment?
% yes = 1, no = 0.
% This is segment-level channel interpolation from the FASTER EEGlab plug-in.
segment_interpolation = 1;


% do you want to do segment rejection (using amplitude and joint probability criteria)?
% yes = 1, no = 0.
segment_rejection = 0;
...
% Select the type of re-referencing you want. Average re-reference (=1)
...
% Select the format to save your processed data at the end of HAPPE!
...
for current_file = 1:length(FileNames)
    ...
    %% Plot data from selected channels before running ICA
    eegplot(EEG.data);
    saveas(gcf,fullfile(src_folder_name ,strrep(FileNames{current_file}, src_file_ext,'_eegplot_beforeICA.jpg')));
    ...
    %% plot data after all preprocessing
    eegplot(EEG.data);
    saveas (gcf, fullfile(src_folder_name, strrep(FileNames{current_file}, src_file_ext,'_eegplot_afterAllPreproc.jpg')));
